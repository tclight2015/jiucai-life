  ...
  <PageLayout>
    ...
    <div className="grid grid-cols-2 gap-3 mb-6">
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">已幫助人數</p>
      </div>
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm text-center">
        <p className="text-2xl font-bold text-primary">$125</p>
        <p className="text-xs text-muted-foreground mt-1">累計還回 USDT</p>
      </div>
    </div>
      ...
      {mockWinners.map((w, i) => (
        <div key={i} className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          ...
          <div className="p-3">
            ...
            {w.note && (
              <p className="text-xs text-foreground/70 mt-1.5 italic">「{w.note}」</p>
            )}
