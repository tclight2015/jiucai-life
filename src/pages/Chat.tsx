...
const initial: Message[] = [
  ...
  },
  {
    ...
    replies: [{ wallet: "官方", text: "我們積極洽談中，消息會第一時間公布。", time: "14:00", official: true }],
  },
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2 mb-1">
                  <span className={`text-xs font-semibold ${msg.official ? "text-primary" : "text-muted-foreground"}`}>{msg.wallet}</span>
                ...
                {msg.replies.map((r, i) => (
                  <div key={i} className={`rounded-lg border px-3 py-2 ${r.official ? "border-primary/30 bg-primary/5" : "border-border bg-muted/40"}`}>
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`text-xs font-semibold ${r.official ? "text-primary" : "text-muted-foreground"}`}>{r.official ? t("common.official") : r.wallet}</span>
